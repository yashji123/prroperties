...
function getDataViewRes(response) {
    ...
    selector.find('.property-type').text(property_type_name(response.property_type));
    selector.find('.checkin-date').text(response.startDate);
    if(response.endDate){
        selector.find('.checkout-date').text(response.endDate);
    }else{
        selector.find('.checkout-date').text('-');
    }
    selector.find('.guests').text(response.guest);
    selector.find('.final-price').text('$'+response.finalPrice);
...
dt = $('#contactDataTable').DataTable({
    ...
    {
        "data": "startDate",
        "name": 'listing_contacts.startDate'
    },
    {
        "data": "endDate",
        "name": 'listing_contacts.endDate'
    },
    {
        "data": "guest",
        "name": 'listing_contacts.guest'
    },
    {
        "data": "finalPrice",
        "name": 'listing_contacts.finalPrice',
...
function property_type_name(id){
    if(id == 1){
        return 'Monthly';
    }else if(id == 2){
        return 'Yearly';
