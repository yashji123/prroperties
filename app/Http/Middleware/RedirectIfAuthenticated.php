...
class RedirectIfAuthenticated
{
    ...
    public function handle(Request $request, Closure $next, ...$guards)
    {
        ...
        foreach ($guards as $guard) {
            if (Auth::guard($guard)->check()) {
                if (Auth::user()->role == USER_ROLE_ADMIN) {
                    ...
                    // If you want to keep the owner on the current page or handle differently
                    // For example, you might want to do nothing and let the request continue
                    return $next($request);
        ...
        return $next($request);
                    ...
                    //  return redirect(route('plan'))->with('success', __('Welcome to your plan!'));
